...
• /cancel \- Отменить все
• /buyalert \<ресурс\> \<макс\_цена\> \<мин\_кол\> \- Групповой алерт покупки

📈 **Торговля \& Статистика:**
• /top\_player \- Ваши сделки, выгода \(24ч/\)
• /top\_list \- Топ\-10 по прибыли
...
"""
    ...
    logger.debug(f"Help text length in bytes: {len(help_text.encode('utf-8'))}")
    logger.debug(f"Sending help text: {repr(help_text)}")
    ...
    bot.reply_to(message, help_text, parse_mode='MarkdownV2', reply_markup=markup)